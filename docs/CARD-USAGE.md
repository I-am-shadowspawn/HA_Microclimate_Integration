...
For the 1.3.0 compact transition, follow [the clean-install runbook](CLEAN-INSTALL-1.3.0.md). If already on the compact series, install 1.4.1 and retain the entry and card configuration. Update the JavaScript module resource to `/microclimate_integration/microclimate-cards.js?v=1.4.1`, then refresh the frontend. Preserve any custom colour configuration.
...
2. In dashboard settings → Resources (enable Advanced Mode in your user profile if necessary), add `/microclimate_integration/microclimate-cards.js?v=1.4.1`, type **JavaScript module**. Add it once. If upgrading a previous card resource, change its version query and reload the browser.
...
- **Version mismatch/custom element missing**: install the entire 1.4.1 integration, register the module resource, restart HA and reload the browser cache.
